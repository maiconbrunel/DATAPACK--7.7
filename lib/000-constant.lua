...
-- Purpose: Unified global constants and helpers
-- Notes: Contains all directions, combat, conditions, items, effects, messages, player flags, skills, colors, mapmarks, CONFIG/STORAGE helpers
...
-- Config and Storage helpers
Config_meta = {
	__index = function(t, k)
		if k == "type" then return function() return "Config" end end
		return getConfigValue(k)
	end,
	__newindex = function(t, k, v) return false end
}
CONFIG = setmetatable({}, Config_meta)

Storage_meta = {
	__index = function(t, k)
		if k == "type" then return function() return "Storage" end end
		return getStorage(k)
	end,
	__newindex = function(t, k, v) return doSetStorage(k, v) end
}
STORAGE = setmetatable({}, Storage_meta)
