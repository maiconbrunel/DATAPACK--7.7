...
-- Purpose: Player login handling
-- Notes: Corrected account manager, death loss, blessings, and creature events
...
function onLogin(cid)
	-- Remove Account Manager if accountManager disabled
	if not getBooleanFromString(getConfigValue('accountManager')) then
		if getCreatureName(cid) == "Account Manager" then
			return doRemoveCreature(cid, true)
		end
	end

	-- Set death loss percentage
	local loss = getConfigValue('deathLostPercent')
	if loss and getPlayerStorageValue(cid, "bless") ~= 5 then
		doPlayerSetLossPercent(cid, PLAYERLOSS_EXPERIENCE, loss * 10)
	end

	-- Apply death blessing if stored
